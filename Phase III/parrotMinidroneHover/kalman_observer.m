...
Q0_1 = Q0multiplier * diag([1e-1,   1e-2]);                    

% SS2: Dropped 4th element (w1) from 1e-3 to 1e-5
Q0_2 = Q0multiplier * diag([9e-1, 3e-1, 2e-1, 1e-3]);     

% SS3: Dropped 4th element (w2) from 1e-3 to 1e-5
Q0_3 = Q0multiplier * diag([9e-1, 2e-1, 2e-1, 1e-3]);    

Q0_4 = Q0multiplier * diag([1e-1,   1e-3]);
%% 4. Measurement noise covariance R0 (from Table 2.1)
% Same state order - C=eye so measurements = states directly

%% 4. Measurement noise covariance R0 (from Table 2.1)
% Same state order - C=eye so measurements = states directly
R0multiplier = 1;   %WAS 1  

R0_1 = R0multiplier * diag([1.8010e-3,  1.8271e-4]);                           

% SS2: Increased 4th element (w1) from 1.8271e-6 to 5.0e-4
R0_2 = R0multiplier * diag([4.2821e-1, 3.4817e-4, 2.2190e-3, 1.8271e-6]); 

% SS3: Increased 4th element (w2) from 4.1449e-6 to 5.0e-4
R0_3 = R0multiplier * diag([7.3221e-1, 2.6060e-4, 2.2190e-3, 1.8271e-6]); 

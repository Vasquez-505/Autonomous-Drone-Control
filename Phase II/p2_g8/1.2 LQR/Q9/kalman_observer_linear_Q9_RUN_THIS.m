...
% Contains all parameters, matrices and model loading needed for Q5, Q6, Q7

% --- Drone parameters ---
drone_model_parameters

% --- State-space matrices (from Phase I) ---
% SS1 - Vertical:           x = [r3; v3],             u = F3'
A1 = [0, 1; 0, 0];
B1 = [0; 1/m];
C1 = eye(2);
D1 = zeros(2, 1);

% SS2 - Lateral-Roll:       x = [r2; th1; v2; w1],   u = M1'
A2 = [0, 0, 1, 0; 0, 0, 0, 1; 0, g, 0, 0; 0, 0, 0, 0];
B2 = [0; 0; 0; 1/J11];
C2 = eye(4);
D2 = zeros(4, 1);

% SS3 - Longitudinal-Pitch: x = [r1; th2; v1; w2],   u = M2'
A3 = [0, 0, 1, 0; 0, 0, 0, 1; 0, -g, 0, 0; 0, 0, 0, 0];
B3 = [0; 0; 0; 1/J22];
C3 = eye(4);
D3 = zeros(4, 1);

% SS4 - Yaw:                x = [th3; w3],             u = M3'
A4 = [0, 1; 0, 0];
B4 = [0; 1/J33];
C4 = eye(2);
D4 = zeros(2, 1);
...
% % --- Pole-placement regulator gains (from Phase I) --- %%%%%%THIS IS
% COMMENTED OUT BECAUSE THE POLES (OPTIMAL LQR POLES) ARE COMPUTED BY
% RUNNING SERVO_CONTROLLER_OF THIS FOLDER

%lqr_servo_controller
...
% fprintf('Observer eigenvalues (optimal Q0, R0):\n');
% fprintf('  SS1 (vertical)      : '); disp(E1.');
% fprintf('  SS2 (lateral)       : '); disp(E2.');
% fprintf('  SS3 (longitudinal)  : '); disp(E3.');
% fprintf('  SS4 (yaw)           : '); disp(E4.');

% --- Observer matrices for Simulink ---
A_obs1 = A1 - L1*C1;   B_obs1 = [B1, L1];
A_obs2 = A2 - L2*C2;   B_obs2 = [B2, L2];
A_obs3 = A3 - L3*C3;   B_obs3 = [B3, L3];
A_obs4 = A4 - L4*C4;   B_obs4 = [B4, L4];

C_obs1 = eye(2);        D_obs1 = zeros(2, 3);
C_obs2 = eye(4);        D_obs2 = zeros(4, 5);
C_obs3 = eye(4);        D_obs3 = zeros(4, 5);
C_obs4 = eye(2);        D_obs4 = zeros(2, 3);

% --- Load Simulink model ---
model_name = 'Linear_model_Kalman_Bucy';

%% Q5 - Observer Convergence
...
colors_Q6 = [0.85 0.33 0.10;    % red    - x0.01
             0.93 0.69 0.13;    % yellow - x0.1
             0.00 0.45 0.70;    % blue   - optimal
             0.47 0.67 0.19;    % green  - x10
             0.49 0.18 0.56];   % purple - x100
...
for k = 1:5
    ...
    [L1,~,~] = lqe(A1, eye(2), C1, Q0_vert*Q0_scales(k), R0_vert);
    [L2,~,~] = lqe(A2, eye(4), C2, Q0_lat *Q0_scales(k), R0_lat);
    [L3,~,~] = lqe(A3, eye(4), C3, Q0_lon *Q0_scales(k), R0_lon);
    [L4,~,~] = lqe(A4, eye(2), C4, Q0_yaw *Q0_scales(k), R0_yaw);
    ...
    results_Q6{k}.y1_meas = out_Q6.y1_meas;
    results_Q6{k}.x1_hat  = out_Q6.x1_hat;
    results_Q6{k}.y2_meas = out_Q6.y2_meas;
    results_Q6{k}.x2_hat  = out_Q6.x2_hat;
    results_Q6{k}.y3_meas = out_Q6.y3_meas;
    results_Q6{k}.x3_hat  = out_Q6.x3_hat;
    results_Q6{k}.y4_meas = out_Q6.y4_meas;
    results_Q6{k}.x4_hat  = out_Q6.x4_hat;
end

% Restore optimal gains
[L1,~,~] = lqe(A1, eye(2), C1, Q0_vert, R0_vert);
[L2,~,~] = lqe(A2, eye(4), C2, Q0_lat,  R0_lat);
[L3,~,~] = lqe(A3, eye(4), C3, Q0_lon,  R0_lon);
[L4,~,~] = lqe(A4, eye(2), C4, Q0_yaw,  R0_yaw);
A_obs1 = A1-L1*C1; B_obs1 = [B1,L1];
A_obs2 = A2-L2*C2; B_obs2 = [B2,L2];
A_obs3 = A3-L3*C3; B_obs3 = [B3,L3];
A_obs4 = A4-L4*C4; B_obs4 = [B4,L4];

% Plot all 5 cases on same figure
%plot_sensitivity(results_Q6, Q0_labels, colors_Q6, 'Q6 - Q_0 Sensitivity Analysis');
...
colors_Q7 = [0.85 0.33 0.10;    % red    - x0.01
             0.93 0.69 0.13;    % yellow - x0.1
             0.00 0.45 0.70;    % blue   - optimal
             0.47 0.67 0.19;    % green  - x10
             0.49 0.18 0.56];   % purple - x100
...
for k = 1:5
    ...
    [L1,~,~] = lqe(A1, eye(2), C1, Q0_vert, R0_vert*R0_scales(k));
    [L2,~,~] = lqe(A2, eye(4), C2, Q0_lat,  R0_lat *R0_scales(k));
    [L3,~,~] = lqe(A3, eye(4), C3, Q0_lon,  R0_lon *R0_scales(k));
    [L4,~,~] = lqe(A4, eye(2), C4, Q0_yaw,  R0_yaw *R0_scales(k));
    ...
    results_Q7{k}.y1_meas = out_Q7.y1_meas;
    results_Q7{k}.x1_hat  = out_Q7.x1_hat;
    results_Q7{k}.y2_meas = out_Q7.y2_meas;
    results_Q7{k}.x2_hat  = out_Q7.x2_hat;
    results_Q7{k}.y3_meas = out_Q7.y3_meas;
    results_Q7{k}.x3_hat  = out_Q7.x3_hat;
    results_Q7{k}.y4_meas = out_Q7.y4_meas;
    results_Q7{k}.x4_hat  = out_Q7.x4_hat;
end
...
function plot_key_outputs(t, y1_meas, x1_hat, y2_meas, x2_hat, ...
    ...
    fs     = 10;
    fs_t   = 11;
    c_meas = [0.00 0.45 0.70];   % blue   - noisy measurement
    c_est  = [0.85 0.33 0.10];   % orange - estimated state

    % --- Figure 1: Key outputs for report (r3, r2, r1, theta3) ---
    ...
    % r3 - vertical position (SS1, state 1)
    ...
    title('Vertical Position $r_3$', 'Interpreter', 'latex', ...
    ...
    % r2 - lateral position (SS2, state 1)
    ...
    xlabel('Time [s]', 'FontSize', fs);
    title('Lateral Position $r_2$', 'Interpreter', 'latex', ...
          'FontSize', fs_t, 'FontWeight', 'bold');
    legend('Location', 'eastoutside', 'FontSize', fs-1, 'Box', 'off');
    grid on; grid minor;
    set(gca, 'Color', 'white', 'GridAlpha', 0.15, 'Box', 'on', 'FontSize', fs);

    % r1 - longitudinal position (SS3, state 1)
    ...
    xlabel('Time [s]', 'FontSize', fs);
    title('Longitudinal Position $r_1$', 'Interpreter', 'latex', ...
          'FontSize', fs_t, 'FontWeight', 'bold');
    legend('Location', 'eastoutside', 'FontSize', fs-1, 'Box', 'off');
    grid on; grid minor;
    set(gca, 'Color', 'white', 'GridAlpha', 0.15, 'Box', 'on', 'FontSize', fs);

    % theta3 - yaw (SS4, state 1)
    ...
    xlabel('Time [s]', 'FontSize', fs);
    title('Yaw $\theta_3$', 'Interpreter', 'latex', ...
          'FontSize', fs_t, 'FontWeight', 'bold');
    legend('Location', 'eastoutside', 'FontSize', fs-1, 'Box', 'off');
    grid on; grid minor;
    set(gca, 'Color', 'white', 'GridAlpha', 0.15, 'Box', 'on', 'FontSize', fs);

    % --- Figure 2: All states SS1 (Vertical) ---
    plot_subsystem(t, y1_meas, x1_hat, ...
        sprintf('%s - Vertical Subsystem (SS1)', prefix), ...
        {'$r_3$ [m]', '$\dot{r}_3$ [m/s]'}, ...
        c_meas, c_est, lw, fs, fs_t);

    % --- Figure 3: All states SS2 (Lateral) ---
    plot_subsystem(t, y2_meas, x2_hat, ...
        sprintf('%s - Lateral Subsystem (SS2)', prefix), ...
        {'$r_2$ [m]', '$\theta_1$ [rad]', '$\dot{r}_2$ [m/s]', '$\dot{\theta}_1$ [rad/s]'}, ...
        c_meas, c_est, lw, fs, fs_t);

    % --- Figure 4: All states SS3 (Longitudinal) ---
    plot_subsystem(t, y3_meas, x3_hat, ...
        sprintf('%s - Longitudinal Subsystem (SS3)', prefix), ...
        {'$r_1$ [m]', '$\theta_2$ [rad]', '$\dot{r}_1$ [m/s]', '$\dot{\theta}_2$ [rad/s]'}, ...
        c_meas, c_est, lw, fs, fs_t);

    % --- Figure 5: All states SS4 (Yaw) ---
    plot_subsystem(t, y4_meas, x4_hat, ...
        sprintf('%s - Yaw Subsystem (SS4)', prefix), ...
        {'$\theta_3$ [rad]', '$\dot{\theta}_3$ [rad/s]'}, ...
        c_meas, c_est, lw, fs, fs_t);
end

% Helper - plot all states of one subsystem
...
% Helper Function - plot sensitivity analysis (one figure per state)

...
function [K, S, E] = my_lqr(A, B, Q, R)
    % Solves continuous control ARE via Hamiltonian eigendecomposition.
    % Drop-in replacement for lqr() without the Control System Toolbox.
    n = size(A,1);
    H = [ A        , -B*(R\B') ;
         -Q        , -A'      ];
    [V, D] = eig(H);
...
function [L, P, E] = my_lqe(A, G, C, Q, R)
    [L, P, E] = my_lqr(A', C', Q, R);  % duality trick
    L = L';
end
%% 5. Gains
...
y_ref   = 2;  %model doesnt work for yref>6 (all other0)
z_ref   = 3;
phi_ref = 0;
